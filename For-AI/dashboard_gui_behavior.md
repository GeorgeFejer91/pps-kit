...
The applet retains the long Segment 0-6 scroll surface, sticky stage headings/actions, and per-stage collapse controls. The independently launched desktop applet omits the hosted page-navigation/status top bar entirely because its sidebar already owns the full `Peripersonal Space Design Toolkit` identity, mode, theme, and workflow navigation. Theme selection is a minimal horizontal sun/moon pill in the sidebar: a single high-contrast indicator sits behind the active line icon and slides between the two states. Do not use nested black/white squares or a text `Dark`/`Light` button. Public Documentation and Downloads navigation remains on the hosted website; the hosted interface uses the same sidebar theme toggle while retaining its page/status top bar. On hosted layouts at `760px` or narrower, page navigation owns a full-width first row, status controls wrap below it, and the header is non-sticky. At `600px` or narrower that header is the first in-flow page surface above the stacked sidebar, with short visible `Experiment`/`Docs` labels that retain full accessible names and do not clip.
