...
The graph is a responsive square in both source/local and compiled/hosted dashboards. `Citation topology` is the default real deterministic force layout: citation neighbours attract, publication prominence softly influences central placement, radius-aware separation prevents overlap, and 22 isolates occupy a stable perimeter. Circle area encodes normalized citations received in the displayed network. `Publication year` is the alternative continuous year-anchored layout and remains collision-free.

All 571 links are faintly visible for context. Selecting a node highlights the local neighbourhood and distinguishes incoming from outgoing citation direction. The Toolkit legend and detail panel preserve four evidence states: 15 runnable, 49 supported but parameter-incomplete, 32 not assessed, and 1 adjacent/scope conflict. Details show title, authors, DOI/source links, abstract provenance or availability caveat, metrics, directional citation neighbours, and every exact-DOI Toolkit record and extracted parameter. The semantic publication list, keyboard navigation, focus return, live status, dark theme, and reduced-motion/mobile behavior remain equivalent accessible inspection paths; citation size and centrality are navigation encodings, never study-quality measures.
