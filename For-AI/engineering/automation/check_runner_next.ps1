...
try {
    if ($Mode -eq "Core") {
        ...
        cargo check --locked -p pps-contracts -p pps-brsp -p pps-runner-core
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        cargo clippy --locked -p pps-contracts -p pps-brsp -p pps-runner-core --all-targets -- -D warnings
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        cargo test --locked -p pps-contracts -p pps-brsp -p pps-runner-core
        exit $LASTEXITCODE
    }
