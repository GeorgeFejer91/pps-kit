...
$Spec = Join-Path $Root "apps\runner\packaging\PPSExperimentRunner.spec"
...
& $Python (Join-Path $Root "For-AI\engineering\release\tools\check_qt_runtime.py") --packaged-runner (Join-Path $Root "dist\PPSExperimentRunner")
if ($LASTEXITCODE -ne 0) {
