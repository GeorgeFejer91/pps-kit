...
ALLOWED_RELEASE_ARCHIVES: dict[Path, str] = {}
ALLOWED_WAV_PREFIXES = {
    Path("packages") / "pps-resources" / "assets" / "breathing",
    Path("packages") / "pps-resources" / "assets" / "click",
    Path("packages") / "pps-resources" / "assets" / "preloads",
    Path("packages") / "pps-resources" / "assets" / "tactile",
}
SPOKEN_ASSETS = [
...
def check_required_files() -> list[str]:
    required = [
        ...
        "CITATION.cff",
        "pyproject.toml",
        "packages/pps-resources/configs/experiment.example.toml",
        "packages/pps-resources/configs/stimulus_design.example.json",
        ...
        "packages/pps-resources/assets/breathing/spoken_assets_manifest.json",
        "packages/pps-resources/assets/breathing/spoken_asset_variants.json",
        "packages/pps-resources/assets/breathing/british_kokoro/spoken_assets_manifest.json",
        "packages/pps-resources/assets/breathing/original_study5/spoken_assets_manifest.json",
        "packages/pps-resources/assets/click/mouse_click_tone_1200Hz_50ms.wav",
        "packages/pps-resources/assets/tactile/default_tactile_cue.wav",
        "packages/pps-resources/assets/master_blocks/Master_Block_1.csv",
        "packages/pps-resources/assets/master_blocks/Master_Block_2.csv",
        "packages/pps-resources/assets/0. Head-Related Impulse Response (HRIR) model/FABIAN_HRIR_measured_HATO_0.sofa",
        "packages/pps-resources/assets/0. Head-Related Impulse Response (HRIR) model/FABIAN_HRIR_measured_HATO_0.manifest.json",
        ...
        "packages/pps-resources/data/sample/audio_tactile_with_facilitation_preregistered_2p5sd.csv",
...
def check_spoken_assets() -> list[str]:
    problems = []
    asset_dir = REPO_ROOT / "packages" / "pps-resources" / "assets" / "breathing"
    ...
    for variant_id in sorted(expected_variants & found_variants):
        ...
        directory = REPO_ROOT / "packages" / "pps-resources" / variant.get("directory", "")
        manifest_path = REPO_ROOT / "packages" / "pps-resources" / variant.get("manifest", "")
...
def check_master_blocks() -> list[str]:
    ...
    for filename in ["Master_Block_1.csv", "Master_Block_2.csv"]:
        path = REPO_ROOT / "packages" / "pps-resources" / "assets" / "master_blocks" / filename
        if not path.exists():
            continue
...
def check_study_templates() -> list[str]:
    problems = []
    template_dir = REPO_ROOT / "packages" / "pps-resources" / "study_templates"
