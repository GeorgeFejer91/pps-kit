...
from peripersonal_space_toolkit.runtime_paths import designer_frontend_root
...
def test_dashboard_static_assets_are_packaged():
    dashboard_files = designer_frontend_root()
    viewer_files = dashboard_files / "viewer"
    ...
    website_root = public_root / "website"
    public_index = (website_root / "index.html").read_text(encoding="utf-8")
    public_docs = (website_root / "documentation" / "index.html").read_text(encoding="utf-8")
    public_download = (website_root / "download" / "index.html").read_text(encoding="utf-8")
    static_version = "20260823-modular-monorepo"
    ...
    assert 'id="download-designer"' in html
    assert 'id="download-runner"' in html
    ...
    assert "Incompatible Shared versions are rejected instead of mixed." in html
...
def test_dashboard_payload_uses_normalized_trajectory_controls_for_render_and_bake():
    app_js = designer_frontend_root().joinpath("app.js").read_text(encoding="utf-8")
...
def test_dashboard_launches_every_finished_profile_from_segment6(tmp_path: Path, monkeypatch):
    ...
    packaged_runner.write_bytes(b"test runner")
...
def test_pfeiffer_preload_loads_bilateral_lateral_trajectories(tmp_path: Path):
    ...
    inventory = json.loads((root / "packages" / "pps-resources" / "assets" / "preloads" / "preload_inventory.json").read_text(encoding="utf-8"))
...
def test_preload_catalog_folders_mirror_dashboard_segments():
    ...
    for profile in inventory["profiles"]:
        profile_dir = root / "packages" / "pps-resources" / "assets" / "preloads" / profile["template_id"]
        ...
        for asset in profile["assets"]:
            path = root / "packages" / "pps-resources" / asset["path"]
            assert path.exists()
