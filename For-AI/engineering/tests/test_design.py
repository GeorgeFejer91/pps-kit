...
def test_packaged_noise_mode_svgs_exist_and_parse():
    for filename in (
        ...
        for asset in (
            ...
            designer_frontend_root().joinpath(filename),
        ):
            assert asset.is_file()
...
def test_pfeiffer_study_profile_controls_trajectory_not_hrtf(tmp_path: Path):
    templates = load_templates(Path(__file__).resolve().parents[3] / "packages" / "pps-resources" / "study_templates")
...
def test_render_loader_accepts_study_profile_json():
    ...
    design = load_render_design(root / "packages" / "pps-resources" / "study_templates" / "pfeiffer_2018_lateral_perihead_left_to_right.json")
...
def test_pfeiffer_style_example_designs_encode_mirrored_looming_parameters(tmp_path: Path):
    root = Path(__file__).resolve().parents[3]
    left = load_design(root / "packages" / "pps-resources" / "configs" / "pfeiffer_style_left_approach.design.json")
    right = load_design(root / "packages" / "pps-resources" / "configs" / "pfeiffer_style_right_approach.design.json")
