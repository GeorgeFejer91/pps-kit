...
def test_compiled_dashboard_and_public_wrappers_share_one_cache_token() -> None:
    ...
    assert version == "20260823-modular-monorepo"

    wrappers = {
        "toolkit": (ROOT / "website" / "index.html").read_text(encoding="utf-8"),
        "documentation": (ROOT / "website" / "documentation" / "index.html").read_text(encoding="utf-8"),
        "downloads": (ROOT / "website" / "download" / "index.html").read_text(encoding="utf-8"),
    }
    for page, wrapper in wrappers.items():
        route = f"app/index.html?page={page}&v={version}"
        assert wrapper.count(route) == 2
        wrapper_versions = set(re.findall(r"app/index\.html\?page=[^&\"]+&v=([^\"]+)", wrapper))
