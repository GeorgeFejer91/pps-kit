...
@pytest.mark.skip(reason="Android companion execution is outside the v1 product boundary")
...
@pytest.mark.skip(reason="Android companion UI is not shipped in v1")
...
def test_runner_output_test_assets_match_expected_routes_and_levels():
    ...
    assert focus_app.OUTPUT_TEST_AUDIO_PATH == repo / "packages" / "pps-resources" / expected_rel
    source_manifest = json.loads(
        (repo / "packages" / "pps-resources" / expected_rel.parent / "stimulus_sources.json").read_text(encoding="utf-8")
    )
...
def test_focus_mode_shell_layout_profile_keeps_controls_visible(tmp_path: Path, available_width: int, available_height: int):
    ...
    assert window.mode_tabs.currentIndex() == window.data_logging_tab_index
    assert window.companion_tab_index == -1
    assert [window.mode_tabs.tabText(index) for index in range(window.mode_tabs.count())] == [
        "Data Logging",
        "Experiment Control",
    ]
    assert window.mode_tabs.isTabEnabled(window.experiment_control_tab_index)
...
def test_launcher_first_screen_is_three_option_environment_gate(tmp_path: Path, monkeypatch):
    ...
    def inspect_launcher() -> None:
        try:
            ...
            assert step_label.text() == "Choose 1 Resume Last Session, 2 Resume Custom Session, or 3 Start New Session."
            ...
            assert dialog.findChild(q["QPushButton"], "sendToPhoneButton") is None
...
@pytest.mark.skip(reason="Android administration is development-only")
