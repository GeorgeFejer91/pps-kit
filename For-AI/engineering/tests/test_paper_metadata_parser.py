...
RESEARCH_TOOLS = ROOT / "For-AI" / "research" / "literature"
sys.path.insert(0, str(RESEARCH_TOOLS))

from tools.paper_metadata_parser import run_audit  # noqa: E402
from tools.paper_metadata_parser.bundle import build_inventory  # noqa: E402
from tools.paper_metadata_parser.parser import (  # noqa: E402
    CONFIDENCE_LABELS,
    EXTRACTION_STATUSES,
    FIELD_STATUSES,
    PDF_STATUSES,
...
AUDIT_DIR = ROOT / "For-AI" / "research" / "literature" / "audiotactile-paper-metadata-audit"
...
def test_paper_metadata_schema_and_status_values_are_valid():
    ...
    tool_schema = json.loads((RESEARCH_TOOLS / "tools" / "paper_metadata_parser" / "schema.json").read_text(encoding="utf-8"))
    audit_records = load_jsonl(AUDIT_DIR / "metadata_audit.jsonl")
...
def test_tracked_audit_files_do_not_include_pdfs_or_extracted_full_text():
    ...
    assert not [
        path
        for path in tracked
        if path.startswith("For-AI/research/literature/audiotactile-paper-metadata-audit/")
