...
DASHBOARD = ROOT / "apps" / "designer" / "frontend"
NETWORK_ASSET = DASHBOARD / "publication_network.v3.json"
LITERATURE_NETWORK = ROOT / "For-AI" / "research" / "literature" / "publication-network"
SOURCE_SNAPSHOT = LITERATURE_NETWORK / "citation_snapshot.v1.json"
CITATION_OVERLAY = (
    LITERATURE_NETWORK / "openalex_audiotactile_citation_overlay.20260808.json"
)
PRIMARY_SOURCE_CITATION_OVERLAY = (
    LITERATURE_NETWORK / "primary_source_citation_overlay.20260808.json"
)
COVERAGE_AUDIT = ROOT / "For-AI" / "research" / "literature" / "preload-ledgers" / "audiotactile_literature_coverage.json"
PRELOAD_INVENTORY = ROOT / "packages" / "pps-resources" / "assets" / "preloads" / "preload_inventory.json"
...
def test_publication_network_generator_rebuild_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [node, str(ROOT / "For-AI" / "research" / "literature" / "tools" / "build_publication_network_asset.mjs"), "--output", str(rebuilt)],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
