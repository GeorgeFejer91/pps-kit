...
SCRIPT_PATH = ROOT / "For-AI/engineering/validation" / "scripts" / "run_ready_profile_runner_smoke.py"
...
def test_ready_profile_runner_smoke_runs_prepared_profile_package():
    smoke = _load_script()
    short_temp_parent = Path(tempfile.gettempdir()).anchor if os.name == "nt" else None
    with tempfile.TemporaryDirectory(prefix="pps-smoke-", dir=short_temp_parent) as directory:
        ...
        assert (output_dir / "ready_profile_runner_smoke_report.json").exists()
        assert (output_dir / "ready_profile_runner_smoke_report.md").exists()
