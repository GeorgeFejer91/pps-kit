...
def _study5_template_by_id(template_id: str):
    ...
    templates = load_templates(root / "packages" / "pps-resources" / "study_templates")
...
def test_dashboard_replaces_saved_non_default_profile_with_study5_for_startup(tmp_path: Path):
    ...
    saved_design = next(
        template.design
        for template in load_templates(root / "packages" / "pps-resources" / "study_templates")
...
def test_unpublished_study5_template_preloads_breathing_assets_and_filmstrip():
    ...
    preload_run_defaults = json.loads(
        (root / "packages" / "pps-resources" / "assets" / "preloads" / "study5_box_breathing_pps" / "05_run_setup" / "run_defaults.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    for clip in design.prestimulus_files:
        path = root / "packages" / "pps-resources" / clip.path
        assert path.exists()
    ...
    for asset in design.noises:
        path = root / "packages" / "pps-resources" / asset.prebaked_path
        assert path.exists()
    ...
    inventory = load_preload_inventory(resource_root)
    asset_status = profile_asset_status(DEFAULT_STUDY_TEMPLATE_ID, inventory=inventory, repo_root=resource_root)
...
def test_study5_instruction_transcript_library_covers_original_instruction_assets():
    ...
    manifest = json.loads(
        (root / "packages" / "pps-resources" / library["source_manifest"]).read_text(
            encoding="utf-8"
        )
    )
