...
Status: working audit for converting published audio-tactile peripersonal-space paradigms into preloadable study profiles. The stricter runnable-profile gate now lives in `docs/PUBLISHED_STUDY_RECREATION_STATUS.md`; the broader literature coverage ledger lives in `For-AI/research/literature/docs/AUDIOTACTILE_PPS_LITERATURE_COVERAGE_AUDIT.md`.
...
| `canzoneri_2013_amputation_prosthesis` | Canonical dynamic PPS measurement across prosthesis states with upper-arm electrical tactile targets and auditory-only catches | T1-T5 at 300, 800, 1500, 2200, 2700 ms from sound onset with 1000 ms pre/post silence; no tactile-only baseline reported | Clinical/prosthesis context is non-blocking for the PPS-task profile; the source's 8 x T1-T5 x IN/OUT wording implies 80 target rows while the same sentence says 76 tactile-target trials | Current profile passes the paper-specific source-to-runner validation with 80 target rows, 76 auditory-only catches, two 78-row blocks, generated WAVs, software wired-loopback sidecars, and tactile-onset mouse-click responses; exact source WAV/gain envelope, participant-level electrical current, voice-key threshold, and clinical session state remain caveats. |
| `serino_2015_toolless_sync_training` | Bimodal IN/OUT target trials plus auditory-only catch trials | T1-T5 at 300, 800, 1500, 2200, 2700 ms; 80 target rows plus 24 catches preserve the reported 77% target rate as 76.9% | Training context is retained as provenance; the runnable validation targets the one-block PPS assessment profile | Paper-specific source-to-runner validation passed with a 104-row assessment block, generated WAV/session package, software wired-loopback sidecar, and mouse-click simulated tactile responses; exact original noise/gain files, participant-level DS7A current calibration, voice-key threshold/latency, and full training execution remain caveats. |
