...
```powershell
.\For-AI/engineering/validation\scripts\run_loopback_calibration.ps1 -EstablishBaseline -Repeats 5
```

For follow-up stability runs against the existing baseline:

```powershell
.\For-AI/engineering/validation\scripts\run_loopback_calibration.ps1 -Repeats 5
```

To validate a session folder that already has block loopback recordings:

```powershell
.\For-AI/engineering/validation\scripts\run_loopback_calibration.ps1 -SessionDir local_data\sessions\P001_YYYYMMDD_HHMMSS
```
