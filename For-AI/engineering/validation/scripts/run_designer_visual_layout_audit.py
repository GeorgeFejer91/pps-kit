...
COMPILED_ROOT = REPO_ROOT / "apps" / "designer" / "frontend" / "compiled"
RESOURCE_ROOT = REPO_ROOT / "packages" / "pps-resources"
COMPILED_PAGE = "index.html"
...
class QuietHandler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path: str) -> str:
        """Serve compiled bytes plus the two public resource URL namespaces."""

        request_path = unquote(urlsplit(path).path)
        aliases = {
            "/assets/preloads": RESOURCE_ROOT / "assets" / "preloads",
            "/study_templates": RESOURCE_ROOT / "study_templates",
        }
...
def serve_repository() -> Iterator[str]:
    handler = partial(QuietHandler, directory=str(COMPILED_ROOT))
                ...
                if case.phone:
                    for page_name in ("documentation", "downloads", "toolkit"):
                        ...
                        if page_name == "downloads":
                            downloads_path = output_dir / f"{case.name}_downloads_page.png"
                            page.locator('[data-page-panel="downloads"]').screenshot(
                                path=str(downloads_path), animations="disabled"
                            )
                            screenshots.append(downloads_path)
