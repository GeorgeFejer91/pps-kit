...
def _build_runner_command(args: argparse.Namespace, *, runner: Path, screenshot_path: Path) -> list[str]:
    if args.runner_mode == "source":
        command = [
            sys.executable,
            str(REPO_ROOT / "apps" / "runner" / "launchers" / "focus_runner_entry.py"),
        ]
    else:
        command = [str(runner)]
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="packaged launches dist/PPSExperimentRunner.exe; source launches apps/runner/launchers/focus_runner_entry.py through Python.",
    )
