...
SCRIPT_PATH = Path("For-AI/experiments/android-companion/validation/validate_android_lsl_runtime_artifact.py")
...
def test_android_lsl_runtime_validator_rejects_stream_description_setup_sha_drift(tmp_path: Path):
    run_dir = tmp_path / "phone-run"
    run_dir.mkdir()
    _write_lightweight_phone_run(run_dir)
    _inject_phone_run_provenance(run_dir)
    ...
    metadata["source_run_setup_sha256"] = "wrong"
    ...
    assert result.ok is False
    assert "Android LSL stream description numeric_triggers.session_metadata_json source_run_setup_sha256 differs" in "\n".join(
        result.failures
    )
...
def test_android_lsl_runtime_validator_rejects_data_max_reconstruction_spine_gap(tmp_path: Path):
    run_dir = tmp_path / "phone-run"
    run_dir.mkdir()
    _write_lightweight_phone_run(run_dir)
    _write_phone_owned_data_export(run_dir)
    data_max_file = tmp_path / "phone_owned_exports" / "2.Data_max" / "P001" / "runs" / run_dir.name / "reconstruction_contract.json"
...
def test_android_lsl_runtime_validator_accepts_phone_run_malformed_command_sample_ack_payload(tmp_path: Path):
    run_dir = tmp_path / "phone-run"
    run_dir.mkdir()
    _write_phone_run_with_native_command_diary(run_dir, row=_phone_native_malformed_command_row())
...
def test_android_lsl_runtime_validator_rejects_phone_run_malformed_command_payload_schema_gap(tmp_path: Path):
    run_dir = tmp_path / "phone-run"
    run_dir.mkdir()
    row = _phone_native_malformed_command_row()
    payload = dict(row["payload"])
    payload.pop("schema")
    row["payload"] = payload
    row["ack_sample"][9] = json.dumps(payload)
    ...
    failures = "\n".join(result.failures)
    assert result.ok is False
    assert "malformed command ack payload schema mismatch" in failures
...
def _phone_native_malformed_command_row() -> dict:
    command_sample = [
        "bad-schema",
        "",
        "",
    ...
    payload = {
        ...
        "rejected_before_handler": True,
        "command": "invalid_lsl_command",
        "malformed_sample_id": "invalid-lsl-command-abc123def456",
        "package_id": "pkg-001",
        "participant_id": "P001",
        "session_id": "part-001",
        "part_session_id": "part-001",
        "session_group_id": "group-001",
        "part_number": "1",
        "raw_sample_channel_count": len(command_sample),
        "expected_channel_count": len(validator.LSL_COMMAND_CHANNELS),
        "raw_sample_schema": "bad-schema",
        "raw_command_id": "",
        "raw_session_id": "",
        "raw_sender_id": "pc_runner",
        "raw_command": "pause",
        "raw_issued_lsl_time": "42.750000000",
        "raw_payload_redacted": True,
        "raw_sample_preview": command_sample[:6] + ["<redacted>"],
        "supported_commands": [
            "start_experiment",
            "start_part",
            "pause",
    ...
    ack_sample = [
        "pps-lsl-command-ack.v1",
        "invalid-lsl-command-abc123def456",
        "part-001",
        "android_phone",
        "rejected",
        "invalid_command_sample",
        "42.760000000",
        "42.770000000",
        "42.780000000",
        json.dumps(payload),
    ...
    return {
        "schema": "pps-android-command-diary.v1",
        "command_id": "invalid-lsl-command-abc123def456",
        "command_source": "native_lsl",
        "sender_id": "",
        "session_id": "part-001",
        "command": "invalid_lsl_command",
        ...
        "target_part_number": "1",
        "run_id": "phone-run-001",
        "received_lsl_time": 42.76,
        "applied_lsl_time": 42.77,
        "ack_lsl_time": 42.78,
        "ack_sent": True,
        "command_channels": [
            "schema",
        ...
        "ack_sample": ack_sample,
        "phone_unix_ms": 1780000000750,
        "phone_elapsed_realtime_ms": 124206,
    }
...
def _inject_phone_run_provenance(
    ...
    *,
    ...
    catalog["reconstruction"]["source_run_setup_sha256"] = "runhash"
    (run_dir / "phone_run_catalog_entry.json").write_text(json.dumps(catalog), encoding="utf-8")
...
def _stream_session_metadata_json(
    *,
    ...
    source_run_setup_sha256: str = "runhash",
    participant_metadata: dict | None = None,
    haptic_capability: dict | None = None,
) -> str:
    payload = {
