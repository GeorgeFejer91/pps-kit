...
- Dashboard orchestration and Segment 0-6 materialization adapters: `src/peripersonal_space_toolkit/dashboard_app.py`. Canonical segment ownership, public contracts, ordered dependencies, and lineage validation live in `designer_segments/{contracts.py,registry.py,segment_0.py,...,segment_6.py}`.
...
- Designer frontend source/build: `dashboard/{index.html,styles.css,app.js,designer_main.js}` plus `dashboard/segment_registry.js` for the mirrored UI workflow graph and `dashboard/designer_api.js` for named backend operations and structured boundary errors; `dashboard/compiled/` is the shared native/GitHub-Pages artifact.
...
- Dashboard backend helper seams: `src/peripersonal_space_toolkit/dashboard_backend/`; `contracts.py` owns safe application errors, `jobs.py` owns the fixed worker pool, queue bound, cancellation, terminal retention, and shutdown lifecycle, and `security.py` owns loopback companion authorization.
...
- Cross-applet profile-to-Runner handoff: `profile_preparation.py` owns profile resolution, public Designer-controller calls, ordered Segment materialization, and the hashed Segment 6 `PreparedProfile` result. Runtime package preparation and participant playback remain in `session_runner.py`, `focus_app.py`, and `focus_launch.py`, with runner-owned LabRecorder RCS subprocess capture isolated in `labrecorder_capture.py`.
