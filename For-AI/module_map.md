...
- Focus Mode phone companion: `runner_companion.py` owns the FastAPI/WebSocket token-gated LAN service and QR payload helpers; `focus_app.py` owns the Qt-thread bridge and snapshot/command state; `android/runner-companion/` owns the native Kotlin/Compose phone app.
