...
DEFAULT_SCHEMA_OUTPUT = REPO_ROOT / "For-AI/research/literature/tools/current_toolkit_input_schema.json"
STUDY_TEMPLATE_DIR = REPO_ROOT / "packages" / "pps-resources" / "study_templates"
...
def _load_templates() -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(STUDY_TEMPLATE_DIR.glob("*.json")):
        ...
        templates[template_id] = {
            **data,
            "_source_file": path.relative_to(REPO_ROOT).as_posix(),
        }
    return templates
...
def build_matrices(output_dir: Path) -> dict[str, Any]:
    ...
    if not study_index_path.exists() or not publication_index_path.exists():
        raise RuntimeError(
            "Run For-AI/research/literature/tools/build_publication_parameter_review_matrix.mjs first; "
