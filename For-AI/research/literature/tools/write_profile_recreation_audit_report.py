...
STATUS_PATH = REPO_ROOT / "packages" / "pps-resources" / "assets" / "preloads" / "profile_recreation_status.json"
MATERIALIZATION_PATH = REPO_ROOT / "local_data" / "profile_recreation_batch" / "materialization_summary.json"
REPORT_PATH = REPO_ROOT / "For-AI" / "research" / "literature" / "docs" / "audit_report.tex"


def main() -> int:
    parser = argparse.ArgumentParser(description="Write the internal profile recreation audit report from the tracked ledger.")
...
def render_audit_report(status: dict[str, Any], materialization: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            ...
            "0--4 dashboard/backend profile. When present, such gaps can include "
            "unreduced speaker-array switching, unreduced Gaussian amplitude fields, "
            "or renderer behavior that cannot be reduced to a reported "
            "trajectory/timing profile."
        ),
        "",
