...
- `Prepare Output Folder for Data Collection` is available only after Segment 6 is prepared. It uses the output folder remembered by `PPSExperimentRunner.exe`, copies the active profile into `study_profile_snapshot/<profile_id>/`, writes `dashboard_runner_bridge_manifest.v1.json`, updates `focus_runner_settings.v1.json`, and appends `output_diary.v1.jsonl`.
