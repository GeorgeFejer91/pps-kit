...
- `Prepare Output Folder for Data Collection` is available only after Segment 6 is prepared. It uses the output folder remembered by `PPSExperimentRunner.exe`, creates `study_profile_snapshot_DO_NOT_DELETE/`, copies the active profile into `study_profile_snapshot_DO_NOT_DELETE/<profile_id>/`, writes `study_profile_snapshot_DO_NOT_DELETE/dashboard_runner_bridge_manifest.v1.json`, updates `focus_runner_settings.v1.json`, and appends `study_profile_snapshot_DO_NOT_DELETE/output_diary.v1.jsonl`.
