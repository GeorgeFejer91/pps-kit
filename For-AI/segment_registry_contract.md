...
- `Save Design and Start Experiment Runner` writes `experiment_block_order.csv` plus `experiment_run_setup_manifest.json`, records the consumed Segment 5 manifest hash, marks the setup prepared, prepares/refreshes the active output folder bridge, materializes the selected participant's native runnable package(s) under the active output folder, and launches Focus Mode through the local companion backend with packaged `PPSExperimentRunner.exe --session-manifest ...`. For new two-part/pre-post preparations this means one `session_group_id` plus `part_01` and `part_02` child packages created up front, not a single uninterrupted two-part runtime session. The normal dashboard launch does not pass operator capture choices; Focus Mode collects those runtime choices before playback. `PPSExperimentRunner.exe` is the only active operator experiment runner; `focus_app.py` imports are internal to the exe and validation harnesses, and direct Python/module launch is retired.
...
- Participant-level block WAV synthesis belongs to the native session-preparation backend, not browser JavaScript. The backend treats Segment 6 as authoritative for playback order, filters it to the selected participant, resolves the referenced Segment 5 final block CSVs, writes one continuous WAV per ordered participant block under `Experiment_context_folder_DO_NOT_DELETE/prepared_blocks/<session_group_id>/part_0N/blocks/` for split packages or `prepared_blocks/<session_id>/blocks/` for legacy unsplit packages, stores package manifests/session metadata under context, and creates sparse participant data folders under the active output/acquisition root as `<participant_id>_<timestamp>/part_01/` and `part_02/` for split packages. Each child manifest remains independently loadable and carries group/sibling metadata so analysis can stitch by participant, run setup, group id, part number, and absolute timestamps.
...
- The native runner UI should make the Segment 6 source and selected participant package inspectable: show the active run setup, participant preparation state, part inventory (`Part 1 complete`, `Part 2 ready`, `Part 2 collected`, or `incomplete`), resolved block order, participant metadata fields, runner-owned capture controls, and an `Open Session Folder` control. It should expose visible run controls and a missed-trial top-up setup checkbox rather than relying only on keyboard shortcuts. When the checkbox is submitted on, part-end top-up blocks auto-play without an additional approval prompt; when submitted off, no top-up ledger/draft/block is produced. `Start Run` must remain disabled until the operator presses `Submit setup`, which validates participant metadata, freezes pre-run capture choices, creates the session/controller metadata layer, and prepares the session/part-lifetime LSL outlets.
- During Segment-prepared playback, the runner must use the participant block CSV sample-position/timing columns as the source of planned trial markers. The audio callback's `audio_sample_zero` event anchors planned trial starts, looming onsets, tactile onsets, response windows, and trial ends. Each split part folder should receive the rolling `<part_session_id>_trials.csv`, the native runner-owned per-part LabRecorder `<part_session_id>_external_labrecorder.xdf` when enabled, per-block `block_XX_audio_evidence.wav`, and optional wired-loopback `block_XX_wired_loopback_input4.wav` files. Verbose local mirrors (`events.csv`, internal `events.xdf`, `lsl_markers.csv`, `lsl_markers.xdf`, trigger dictionary, session metadata, timing QC, LabRecorder capture report files, part completion status, and exploratory/model analysis files) are context or `Data_Analytics` outputs under the matching `<session_group_id>/part_0N/` leaf rather than participant-folder clutter. The optional local audio evidence WAV records the runner's mixed output buffers; physical loopback remains an internal validation reference, not a normal participant-run dependency. The LSL marker outlet is part-session infrastructure for split packages: it must be created once before the first instruction/block of that part and remain online through the end of that part session. If external LabRecorder capture is enabled, the runner must wait for the part-session LSL `source_id` values, start one minimized `LabRecorder.exe` child process through LabRecorder RCS before playback, and stop it at part teardown; block identity and stitching identity come from markers including `session_group_id`, `part_session_id`, and `part_number`. Legacy unsplit manifests may still use one continuous session/XDF and the old in-session Part 2 gate.
