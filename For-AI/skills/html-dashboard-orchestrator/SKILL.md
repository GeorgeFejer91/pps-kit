...
3. Keep the UI lab-oriented.
   ...
   - Segment 0 is the exception to explicit artifact buttons: keep one grouped profile selector, one `Start New Custom Design` action, and one contextual profile-information card. Selecting an existing profile activates its fixed context without an Apply button; starting blank requires naming first and automatically creates its dedicated researcher-workspace folder. Do not show direct Refresh, Apply/Save, Open Folder, duplicate custom-project selector, or data-acquisition-folder controls in Segment 0. Its About modal links `Template Directory` to the physical local directory or hosted GitHub directory and labels its final section `Output`. Segment 1-4 registry generation remains button-owned and explicit: Segment 1 uses `Bake Ingredient`, Segment 2 uses `Bake Trial Sequences`, Segment 3 uses `Bake Baseline/Tactile Trials`, and Segment 4 uses `Bake Trial Pool CSV`. Those stages keep their `Open Folder` actions backed by `/api/state.project_segments`. Registry state stays in backend state/manifests plus compact status chips and previews; do not add generic registry feedback panels with `Status`/`Ingredients`/`Variants`/`WAVs`/`Folder`/`Message` summary rows to any segment. Do not let Segment 1 secretly bake Segment 2, do not let Segment 3 auto-bake Segment 2, and do not let Segment 4 auto-bake Segment 3; downstream segments must fail clearly if upstream manifests are missing or stale.
