...
func InstallFromManifest(ctx context.Context, options InstallOptions, progress ProgressFunc) (InstallResult, error) {
	...
	if strings.TrimSpace(options.ExpectedComponent) != "" && manifest.Component.ID != options.ExpectedComponent {
		return InstallResult{}, fmt.Errorf("downloader is pinned to component %q, manifest provides %q", options.ExpectedComponent, manifest.Component.ID)
	}
	...
	if pathExists(installDir) && !options.Force {
		if err := verifyCompatibleComponentInstall(installDir, manifest); err != nil {
			return InstallResult{}, fmt.Errorf("install dir already exists and cannot be combined: %w", err)
		}
		mergeExisting = true
	}
	...
	if mergeExisting {
		if err := mergeCompatibleTree(partialDir, installDir); err != nil {
			_ = os.RemoveAll(partialDir)
			return InstallResult{}, err
		}
		_ = os.RemoveAll(partialDir)
	...
	if err := writeInstallMarker(installDir, manifest, payload); err != nil {
		return InstallResult{}, err
	}
...
type installedComponents struct {
	...
	SharedManifestSHA256 string                        `json:"shared_manifest_sha256"`
	Components           map[string]installedComponent `json:"components"`
}

func componentMarkerPath(root string) string {
	return filepath.Join(root, "pps_components.json")
}

func readInstalledComponents(root string) (installedComponents, error) {
	var marker installedComponents
	data, err := os.ReadFile(componentMarkerPath(root))
	if err != nil {
		return marker, err
	}
	...
	if marker.Schema != "pps-local-components.v1" {
		return marker, fmt.Errorf("unsupported local component marker %q", marker.Schema)
	}
	if marker.Components == nil {
		marker.Components = map[string]installedComponent{}
	}
	return marker, nil
}

func verifyCompatibleComponentInstall(root string, manifest DownloadManifest) error {
	if strings.TrimSpace(manifest.Component.ID) == "" {
		return errors.New("download manifest has no component identity")
	}
	marker, err := readInstalledComponents(root)
	if err != nil {
		return fmt.Errorf("existing install has no compatible component marker: %w", err)
	}
	if marker.SharedVersion != manifest.Component.SharedVersion ||
		!strings.EqualFold(marker.SharedManifestSHA256, manifest.Component.SharedManifestSHA256) {
		return fmt.Errorf(
			"Shared component mismatch (installed %s/%s, requested %s/%s)",
			marker.SharedVersion, marker.SharedManifestSHA256,
			manifest.Component.SharedVersion, manifest.Component.SharedManifestSHA256,
		)
	}
	if existing, ok := marker.Components[manifest.Component.ID]; ok &&
		(existing.Version != manifest.Component.Version || !strings.EqualFold(existing.ManifestSHA256, manifest.Component.ManifestSHA256)) {
		return fmt.Errorf("component %s is already installed at an incompatible version", manifest.Component.ID)
	}
	return nil
}

func mergeCompatibleTree(sourceRoot string, destinationRoot string) error {
	return filepath.Walk(sourceRoot, func(source string, info os.FileInfo, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		...
		if pathExists(destination) {
			sourceHash, sourceErr := fileSHA256(source)
			destinationHash, destinationErr := fileSHA256(destination)
			if sourceErr != nil || destinationErr != nil || !strings.EqualFold(sourceHash, destinationHash) {
				return fmt.Errorf("refusing incompatible file overlap at %s", relative)
			}
			return nil
		}
...
func writeInstallMarker(root string, manifest DownloadManifest, payload Payload) error {
	...
	if err := os.WriteFile(filepath.Join(root, "pps_install_manifest.json"), append(data, '\n'), 0o644); err != nil {
		return err
	}
	if strings.TrimSpace(manifest.Component.ID) == "" {
		return nil
	}
	components, err := readInstalledComponents(root)
	if err != nil {
		components = installedComponents{
			Schema:               "pps-local-components.v1",
			SharedVersion:        manifest.Component.SharedVersion,
			SharedManifestSHA256: manifest.Component.SharedManifestSHA256,
			Components:           map[string]installedComponent{},
		}
	}
	components.Components[manifest.Component.ID] = installedComponent{
		Version: manifest.Component.Version, ManifestSHA256: manifest.Component.ManifestSHA256, PayloadSHA256: payload.SHA256,
	}
	componentData, err := json.MarshalIndent(components, "", "  ")
	if err != nil {
		return err
	}
	return os.WriteFile(componentMarkerPath(root), append(componentData, '\n'), 0o644)
}
...
func resolveLaunchPath(root string, manifest DownloadManifest) string {
	for _, kind := range []string{"designer", "experiment_runner", "dashboard"} {
		if entrypoint, ok := manifest.Entrypoint(kind); ok && entrypoint.Path != "" {
