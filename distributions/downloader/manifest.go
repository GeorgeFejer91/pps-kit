...
var defaultPayloadKind = "full_windows_x64"
...
type DownloadManifest struct {
	...
	Component            ComponentSelection   `json:"component"`
...
func (m DownloadManifest) Validate() error {
	...
	if strings.TrimSpace(m.Component.ID) != "" {
		if strings.TrimSpace(m.Component.Version) == "" || strings.TrimSpace(m.Component.SharedVersion) == "" {
			return errors.New("component and shared versions are required")
		}
		if len(m.Component.ManifestSHA256) != 64 || len(m.Component.SharedManifestSHA256) != 64 {
			return errors.New("component manifests must include 64-character SHA256 values")
		}
	}
	if len(m.Payloads) == 0 {
