...
| `canzoneri_2013_amputation_prosthesis.json` | partial | Clinical amputation/prosthesis PPS profile preserving the reported 3000 ms dynamic IN/OUT pink-noise task, 1000 ms pre/post silence, T1-T5 delays, upper-arm electrical tactile target context, auditory-only catches, and two-block runner contract. | Paper-specific validation passes with the coherent 80-target + 76-catch runnable contract; the source's literal 76-target wording is retained as a count caveat, and exact pink-noise/gain files, participant-level current calibration, voice-key threshold, clinical group, prosthesis-worn state, and tested-limb assignment remain provenance/context caveats. |
...
| `taffou_2014_cynophobic_rear_looming.json` | partial | Rear-field dog/sheep binaural looming task with Tbefore/T1-T5/Tafter timing and rear-left/rear-right source assets. | Exact dog/sheep source audio, Audacity matching settings, and LISTEN HRTF subject/rendering provenance are not bundled. |
...
| `serino_2015_toolless_sync_training.json` | verified | Synchronous/asynchronous far-audio and hand-tactile training context with a runnable Canzoneri-family PPS assessment block: 3000 ms IN/OUT pink-noise, T1-T5 at 300/800/1500/2200/2700 ms, 80 tactile-target rows, and 24 auditory-only catches. | Exact original noise/gain files, participant-level DS7A current calibration, voice-key threshold/latency, and full training-intervention execution remain provenance caveats. |
...
| `noel_2015_walking_full_body_action.json` | verified | Walking/treadmill PPS profile preserving standing/walking x looming/receding white-noise trials, T1-T5 tactile timings, distance-at-tactile mappings, tactile-only baselines, sound-only catches, and the reported 512-row formula. | Exact physical treadmill, optic-flow display, SPL field, and two-array speaker interpolation remain apparatus/provenance caveats outside software recreation. |
