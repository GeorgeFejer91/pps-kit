...
The additional bundled profile `study5_box_breathing_pps_pink_white` is a Study 5 lab variant created through the dashboard Edit-mode source-removal workflow. It keeps only the original pink and white frontal looming WAVs and prunes blue and brown from the Segment 2 decision pool, while preserving the original Study 5 timing, SOA, baseline/catch, instruction, trajectory, block, participant, and total trial-budget logic. Because the source pool is reduced from four noises to two, Segment 4 family repetitions are doubled to audio-tactile `6.0`, baseline `3.0`, and catch `6.0`; this keeps the prepared pool at 204 rows and six 34-trial blocks.
