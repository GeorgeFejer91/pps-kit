...
If the current shell cannot resolve `go` immediately after install, start a fresh terminal or rely on `windows\Build_PPS_Downloader.ps1`, which also checks `C:\Program Files\Go\bin\go.exe`.
...
powershell -ExecutionPolicy Bypass -File windows\Build_Dashboard_Launcher_Exe.ps1
powershell -ExecutionPolicy Bypass -File windows\Build_PPS_Distribution.ps1 -Version 0.1.0 -ZenodoPayloadUrl "https://zenodo.org/records/<record>/files/PPS-Toolkit-v0.1.0-offline-lab-windows-x64.zip?download=1" -ZenodoDoi "10.5281/zenodo.<record>"
```
