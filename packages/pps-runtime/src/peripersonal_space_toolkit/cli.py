...
def run(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        "`dist\\PPSExperimentRunner\\PPSExperimentRunner.exe` or `apps\\runner\\launchers\\Launch_Experiment_Runner.bat`."
    )
...
def analyze(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--sample", action="store_true", help="Use the bundled deidentified sample dataset.")
    ...
    if args.sample or input_path is None:
        input_path = resource_root() / "data" / "sample" / "audio_tactile_with_facilitation_preregistered_2p5sd.csv"
