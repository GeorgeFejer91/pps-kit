...
| BRM methods/software article framing | `main.tex` includes a methods/software title, abstract, `Contribution and Journal Fit`, a same-journal comparator table, `PPS Toolkit Overview`, `Design, Run, and Recreation Pathway`, technical method, validation/use-case, discussion, declarations, and open-practices sections. | partially supported | Replace placeholder author email/affiliation/funding/conflict fields and perform final human copy edit. |
| Same-journal sensory-suite positioning | `brm_comparator_articles.csv` and the manuscript comparator table now include BRM models for multimodal sensory suites and calibration/toolbox workflows, including browser tactile delivery, PsySuite, cognitive-hearing OpenSesame, Titta, and Mousetrap. | source-ready | Keep this row aligned with any future changes to the suite claim; do not use comparator papers as evidence that PPS Toolkit has completed all route-specific validation. |
...
| Paragraph parsimony and reader flow | The latest self-review pass split dense prose paragraphs so most ordinary prose now carries one primary idea, with remaining long source blocks mostly table rows. `critical_review.md` records the paragraph-density check and residual copy-edit caveat. | source-ready | Re-run a human copy edit after final figures, release links, declarations, and validation artifacts are inserted. |
| Claim boundaries and overclaim prevention | `claim_boundary_audit.csv` maps major manuscript claims to the evidence needed, current source evidence, safe phrasing, unsafe phrasing, and next required artifacts. | source-ready | Keep this ledger aligned with any new manuscript claim; stronger claims still require the listed artifacts. |
...
| Consensus-backed literature grounding | `references.bib`, `evidence_matrix.csv`, `brm_comparator_articles.csv`, and `critical_review.md` record literature-backed rows for PPS, baselines, tactile calibration, HRTF/spatial rendering, software methods, randomization, analysis decisions, and BRM article-style comparators. | source-ready | If new claims are added, add new source-pointer rows rather than making uncited prose claims. |
...
| Current dashboard source rechecked against control ledger | `dashboard_control_recheck_20260630.md` records a point-in-time source recheck of the current local dashboard worktree. The only detected dirty-dashboard label/status change was a not-ready readiness-badge glyph/styling change, already covered by the non-method left-rail readiness-badge row. | source-ready | Rerun after the dirty dashboard changes are committed, replaced, or extended; this is not screenshot or mouse-click usability validation. |
...
| Figure/source-material plan | `main.tex` and `figure_source_plan.csv` now specify the recommended figure set, source artifacts, supported claim, and boundary for workflow, design-matrix, stimulus, evidence-tier, operator-safeguard, and exploratory-analysis figures. | partially supported | Generate final figures from redistributable toolkit artifacts, screenshots, schemas, and validation outputs; do not use copyrighted source-paper figures or private participant data. |
| Procedural gap register | `main.tex` and `procedural_gap_register.csv` now name the remaining procedural methods that must be incorporated or caveated: auditory eligibility/listening checks, route-specific timing, tactile mechanical/perceptual delivery, spatial-audio perception/HRTF assumptions, rights/profile recreation, and analysis governance. | partially supported | Close these rows with final artifacts or keep the associated claims explicitly caveated in the submission. |
