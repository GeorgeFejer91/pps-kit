...
def test_paper_audit_package_summarizes_core_pipeline_without_source_artifacts():
    ...
    assert summary["category_counts"]["covered_runnable_profile"] == 11
    assert len(summary["runnable_profile_records"]) == 11
    assert len(summary["missing_parameter_records"]) == 28
    assert len(summary["toolkit_structure_gap_records"]) == 31
