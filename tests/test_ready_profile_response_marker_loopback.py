...
def test_ready_profile_response_marker_loopback_recovers_smoke_markers(tmp_path: Path):
    ...
    assert report["summary"]["total_expected_markers"] == 1
    assert report["summary"]["total_detected_markers"] == 1
    assert report["summary"]["total_synthetic_recordings"] == 1
    ...
    assert profile["expected_marker_count"] == 1
    assert profile["detected_marker_count"] == 1
    assert profile["detection_rate"] == 1.0
    assert len(profile["synthetic_recordings"]) == 1
