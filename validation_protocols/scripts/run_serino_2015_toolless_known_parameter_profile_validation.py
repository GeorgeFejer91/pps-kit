"""Validate the Serino/Canzoneri 2015 toolless-training profile end to end.

This paper-specific validator starts from the public Frontiers source review,
loads the profile through DashboardController, bakes Segments 2-6,
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "artifacts"
    / "validation_runs"
    / "current_goal_serino_2015_toolless_known_parameter_20260715"
)
EVIDENCE_BOUNDARY = (
    "This validates the software-known parameter contract for Serino, "
    "Canzoneri et al. (2015) toolless synchronous/asynchronous training: "
    "public Frontiers source parameters, 3000 ms pink-noise IN/OUT sounds, "
    "right-hand electrical tactile target context, T1-T5 tactile delays, "
    "one assessment block, 77% tactile-target trials represented as 80 "
    "factorial target rows plus 24 auditory-only catches, Segment 0-6 GUI "
    "materialization, generated WAV packages, software wired-loopback "
    "sidecars, and mouse-click simulated participant-like responses to "
    "tactile targets. The paper reports the tactile-target percentage rather "
    "than an exact catch count, so the runnable profile uses the inherited "
    "Canzoneri-family T1-T5 x IN/OUT x 8 target structure and the nearest "
    "integer catch count preserving the reported 77% target rate. This does "
    "not claim exact original pink-noise/gain files, participant-level DS7A "
    "current calibration, voice-key latency equivalence, execution of the "
    "training intervention schedule, collected human behavior, or the "
    ...
    report = {
        ...
        "observed_vs_expected_outcome": {
            ...
            "observed_emulated_outcome": (
                ...
                "the source-derived measurement-block contract. The emulated run cannot estimate "
                "training-induced PPS plasticity or participant behavior."
            ),
        },
        ...
        "report_json": str(output_dir / "serino_2015_toolless_known_parameter_validation_report.json"),
        "report_md": str(output_dir / "serino_2015_toolless_known_parameter_validation_report.md"),
    }
...
def _expected_contract_from_manual_review() -> dict[str, Any]:
    ...
    return {
        ...
        "source_url": "https://www.frontiersin.org/journals/behavioral-neuroscience/articles/10.3389/fnbeh.2015.00004/full",
        ...
        "tactile_site": "right hand",
        "tactile_modality": "electrical_stimulation_above_threshold",
        "target_repetitions_per_delay_direction": 8,
        "target_trial_count": 80,
        "reported_target_trial_probability": 0.77,
        "catch_trial_count": 24,
        "source_count_caveat": (
            "Frontiers Methods report tactile stimulation in 77% of trials and no tactile "
            "stimulation in the remaining catch trials. The exact catch count is not printed; "
            "the profile uses the inherited Canzoneri-family 80 target rows and 24 catches "
            "because 80/104 = 76.9%."
        ),
        "baseline_trial_count": 0,
        "total_runnable_trial_count": 104,
        "block_count": 1,
        "expected_response_counts": {"respond": 80, "withhold": 24},
        "expected_scientific_outcome": (
            "Synchronous pairing of tactile hand stimulation with far auditory stimulation is expected to "
            "extend hand PPS without tool use; asynchronous pairing is not expected to produce the same "
            "extension. The software validator can test the runnable assessment-block contract, not the "
            "human training-induced PPS plasticity effect."
        ),
    }
...
def _criteria(
    ...
    return {
        ...
        and observed["segment4_audio_tactile_count"] == expected["target_trial_count"]
        ...
        "block_csv_matches_family_counts": counts["family"] == {"audio_tactile": 80, "catch": 24},
        "block_csv_preserves_response_contract": counts["expected_response"] == expected["expected_response_counts"]
        and counts["target_role"] == {"catch_no_target": 24, "tactile_target": 80},
        ...
        and counts["catch_sequence_variant_key"] == {
            "pink_moving_sound": 12,
            "pink_moving_sound_receding": 12,
        },
        ...
        and observed["event_counts"].get("tactile_onset", 0) == expected["target_trial_count"]
        ...
        and observed["participant_outcome_counts"] == {"Hit": 104}
        and observed["participant_expected_response_counts"] == expected["expected_response_counts"]
        and observed["participant_response_given_counts"] == {"false": 24, "true": 80},
...
def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Serino/Canzoneri 2015 Toolless Training Known-Parameter Validation",
        "",
    ...
    lines.extend(
        [
            ...
            "## Source Count Caveat",
            "",
            f"- {report['expected_contract']['source_count_caveat']}",
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Serino/Canzoneri 2015 toolless-training known-parameter GUI/runner validation."
    )
