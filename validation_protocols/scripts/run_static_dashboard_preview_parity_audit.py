...
DIRECT_DASHBOARD_PATH = "src/peripersonal_space_toolkit/dashboard/index.html"
AUDIT_QUERY = {
    "page": "toolkit",
...
def run_audit(
    *,
    ...
    status = load_profile_recreation_status(REPO_ROOT)
...
def _load_preload_inventory() -> dict[str, Any]:
    return json.loads((REPO_ROOT / "assets" / "preloads" / "preload_inventory.json").read_text(encoding="utf-8"))


def _load_template_json(template_id: str) -> dict[str, Any]:
    return json.loads((REPO_ROOT / "study_templates" / f"{template_id}.json").read_text(encoding="utf-8"))
